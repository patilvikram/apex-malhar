...
if [ -f "$MVN_GENERATED_PATH" ]; then
  ...
  DT_CORE_JAR="$BUILD_DIR/malhar-python-3.7.0-SNAPSHOT.jar"
...
echo "Starting Apex Runner..."
if [ ! -x "$DT_HADOOP" ]; then
  echoerr "Warning: hadoop executable not found.  Running standalone with ${DT_JAVA:-java}."
  export CLASSPATH=$DT_CLASSPATH
  "${DT_JAVA:-java}" $DT_CLIENT_OPTS org.apache.apex.malhar.python.ApexPythonRunner "$@" >$APEXRUNNERLOG 2>&1 &
  APEXRUNNERPID="$!"  
else
  ...
  "$DT_HADOOP" org.apache.apex.malhar.python.ApexPythonRunner "$@" >$APEXRUNNERLOG 2>&1 &
  APEXRUNNERPID="$!"
fi
...
PYTHONPATH=$PYTHONPATH:"${script_dir}/../python" ipython "$@"
